 ...
 */
export async function fetchPlacesByRegion(_areaCode: string): Promise<TourApiRawItem[]> {
  const apiKey = process.env.TOUR_API_KEY;
  if (!apiKey) {
    throw new Error('Missing TOUR_API_KEY in environment');
  }

  throw new Error(`Not implemented: fetchPlacesByRegion (base url: ${TOUR_API_BASE_URL})`);
}
