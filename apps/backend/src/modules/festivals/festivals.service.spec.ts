...
describe('sortByImminence', () => {
  ...
  it('같은 날이면 지방 축제를 앞에 둔다', () => {
    // 제안서의 "소규모 지역 특화, 숨은 축제 우선"이 구현되는 지점이다.
    // popularity로는 못 한다 - 실데이터가 전 행 0이라 신호가 없다.
    const rows = [
      festival('서울 축제', '2026-09-05', '2026-09-06', {
        addr1: '서울특별시 종로구',
      }),
      festival('군 축제', '2026-09-05', '2026-09-06', {
        addr1: '충청남도 예산군',
      }),
    ];
  ...
  it('지방이라도 상설 프로그램은 오늘 열리는 대도시 축제보다 뒤다', () => {
    // 지방 우선이 상설 프로그램을 끌어올리면 안 된다. 상설 여부를 먼저 가른다.
    const rows = [
      festival('지방 상설', '2026-01-01', '2026-12-31', {
        addr1: '충청남도 예산군',
      }),
      festival('서울 이틀', TODAY, '2026-09-05', {
        addr1: '서울특별시 종로구',
      }),
    ];
